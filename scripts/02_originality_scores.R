#############################################################################################
# Script to calculate originality scores for mammals, birds, and plants
# original script by Marine Robuchon
# modifications by: Vanessa Rezende, Sandrine Pavoine
#############################################################################################
...
mamtax <- read.csv(paste0(getwd(), "/data/taxonomy_mamPhy_5911species.csv")) # taxonomic info on mammal species in phylogenetic trees
...
## load trait data - these are the 5 traits with the highest coverage from the GIFT database
planttraits <- read.table("./data/traits_to_run.csv", sep=";", header=T, row.names=1 ) 
dim(planttraits)

Toremove <- (1:262466)[apply(planttraits[1:6], 1, function(x) all(is.na(x)))]  # remove species with no data at all
...
write.csv(oriplantsmiss, "./outputs/oriplantsmiss.csv") # save results
